...
if (isset($_GET['id'])){
    ...
    if (isset($_POST['submit'])){
        ...
        if (is_numeric($_POST['id'])){
            ...
            if ($ilosc == ''){
                ...
                if ($stmt = $polaczenie->prepare("UPDATE woda SET ilosc = ? WHERE id = ?")){
        ...
        if (is_numeric($_GET['id']) && $_GET['id'] > 0 ){
            ...
            if ($stmt = $polaczenie->prepare("SELECT `id`, `ilosc` FROM woda WHERE id = ?")){
    ...
    if (isset($_POST['submit'])){
        ...
        if ($ilosc == ''){
            ...
            if ($stmt = $polaczenie->prepare("INSERT woda (ilosc) VALUES (?)")){
