<!DOCTYPE>
<html lang="pl">
	<head>
		<meta charset="utf-8" />
		<meta name='robots' content='noindex,follow' />
		<title>ZGO Magazyn</title>
		<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
		<script src="../js/jquery-3.2.1.slim.min.js"></script>
		<script src="../js/bootstrap.bundle.min.js"></script>
		<link rel="stylesheet" href="../style/custom.css">
		<link rel="stylesheet" href="../style/bootstrap.min.css">
	</head>

	<!-- Navbar -->
	<nav class="navbar navbar-light navbar-expand-md bg-light">
		<div class="container">
			<a href="../index.php" class="navbar-brand">Ekomax Sp. z o.o.</a>
			<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navContent">
				<span class="navbar-toggler-icon"></span>
			</button>
			<div class="collapse navbar-collapse" id="navContent">
				<div class="navbar-nav">
					<a href="../index.php" class="nav-item nav-link">Strona główna</a>
					<a href="../oleje/magazyn.php" class="nav-item nav-link active">Magazyn olejów</a>
					<a href="../odpady/magazyn.php" class="nav-item nav-link">Magazyn odpadów</a>
					<a href="../logowanie/logout.php" class="nav-item nav-link">Wyloguj</a>
				</div>
			</div>
		</div>
	</nav>
	
	<body>
		<div class="jumbotron jumbotron-fluid bg-primary text-white text-center pt-5 pb-5">
			<div class="container">
				<h1 class="display-3">Magazyn olejów</h1>
			</div>
		</div>